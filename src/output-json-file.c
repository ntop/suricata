...
#if 0
      /* Disabled as Suricata already supports them */
      jb_set_string(js, "ja3", f->ndpi_flow->protos.tls_quic.ja3_client);
      jb_set_string(js, "ja3s", f->ndpi_flow->protos.tls_quic.ja3_server);
      jb_set_string(js, "ja4", f->ndpi_flow->protos.tls_quic.ja4_client);
#endif
...
void ndpiJsonBuilder (Flow *f, JsonBuilder *js, ThreadVars *tv) {
  ...
  case NDPI_PROTOCOL_BITTORRENT:
	...
	j += 2, n += f->ndpi_flow->protos.bittorrent.hash[i];
      }
  ...
  case NDPI_PROTOCOL_HTTP_PROXY:
    ...
    if(f->ndpi_flow->http.url != NULL) {
      ndpi_risk_enum risk = ndpi_validate_url(f->ndpi_flow->http.url);
      if (risk != NDPI_NO_RISK)
	{
	  NDPI_SET_BIT(f->ndpi_flow->risk, risk);
	}
