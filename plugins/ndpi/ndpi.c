...
static void OnFlowUpdate(ThreadVars *tv, Flow *f, Packet *p, void *_data)
{
    ...
    if (!flowctx->detection_completed && ip_ptr != NULL && ip_len > 0) {
        ...
        flowctx->detected_l7_protocol = ndpi_detection_process_packet(threadctx->ndpi, flowctx->ndpi_flow,
                ip_ptr, ip_len,
                time_ms, NULL);
...
static bool nDPIProtocolDataHasConflicts(const DetectnDPIProtocolData *us, const DetectnDPIProtocolData *them)
{
...
static int DetectnDPIRiskSetup(DetectEngineCtx *de_ctx, Signature *s, const char *arg)
{
    ...
    if (SigMatchAppendSMToList(
                de_ctx, s, ndpi_risk_keyword_id, (SigMatchCtx *)data, DETECT_SM_LIST_MATCH) == NULL) {
        goto error;
    }
    return 0;
